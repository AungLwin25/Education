...
# Load file
Ben<- function(Data,SheetName,Saved,Month) {
...
dim(raw) -> dimensions
  ...
  d2 %>% mutate_at(vars(1,14:lastcol-1),as.numeric) -> d3
  # +1 to count column index
...
# Summarise numeric variables
  d3[1:maxrow,]-> d4
  d4 %>% summarise(across(14:lastcol-1, ~ sum(.x, na.rm = TRUE))) -> sum
